...
//２．データ表示SQL作成
$stmt = $pdo->prepare("SELECT * FROM gs_db3_table;");
$status = $stmt->execute();
...
if($status==false) {
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){ 
    ...
    $view .= "<p>";
    $view .= $result["id"].":".$result["category1"].":".$result["category2"].":".$result["category3"].":".$result["category4"].":".$result["category5"].":".$result["category6"];
    $view .= "</p>";
  }
}
